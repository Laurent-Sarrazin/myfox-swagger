...
@implementation SWGDeviceWithStateCollection
        ...
        if([items_dict isKindOfClass:[NSArray class]]) {
            ...
            if([(NSArray*)items_dict count] > 0) {
                for (NSDictionary* dict in (NSArray*)items_dict) {
                    SWGDeviceWithState* d = [[SWGDeviceWithState alloc] initWithValues:dict];
                    [objs addObject:d];
                }
...
-(NSDictionary*) asDictionary {
    ...
    if(_items != nil){
        if([_items isKindOfClass:[NSArray class]]){
            ...
            for( SWGDeviceWithState *items in (NSArray*)_items) {
